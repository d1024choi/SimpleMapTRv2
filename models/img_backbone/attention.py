...
def auto_fp16(apply_to=(), out_fp32=False):
    """
    Custom decorator to replicate mmcv's auto_fp16 functionality.
    
    Args:
        apply_to: tuple of argument names to convert to float16
        out_fp32: if True, convert output back to float32
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            # Get function signature to map argument names to values
            sig = inspect.signature(func)
            bound_args = sig.bind(self, *args, **kwargs)
            bound_args.apply_defaults()
            
            # Convert specified arguments to float16
            ...
            for i, param_name in enumerate(param_names[:len(args)]):
                if param_name in apply_to and i < len(args):
                    if isinstance(args[i], torch.Tensor) and args[i].dtype == torch.float32:
                        new_args[i] = args[i].half()
            
            # Convert keyword arguments
            new_kwargs = kwargs.copy()
            for arg_name in apply_to:
                if arg_name in new_kwargs:
                    if isinstance(new_kwargs[arg_name], torch.Tensor) and new_kwargs[arg_name].dtype == torch.float32:
                        new_kwargs[arg_name] = new_kwargs[arg_name].half()
            
            # Call the original function with converted arguments
            result = func(self, *new_args, **new_kwargs)
            
            # Convert output back to float32 if requested
            if out_fp32:
                if isinstance(result, tuple):
                    result = tuple(r.float() if isinstance(r, torch.Tensor) and r.dtype in [torch.float16, torch.bfloat16] else r for r in result)
                elif isinstance(result, torch.Tensor) and result.dtype in [torch.float16, torch.bfloat16]:
                    result = result.float()
...
class FlashAttention(nn.Module):
    ...
    def forward(self, q, kv, 
        ...
        """
        # Note: dtype conversion is handled by @auto_fp16 decorator
        # Ensure tensors are on CUDA and in half precision
        if q.dtype not in [torch.float16, torch.bfloat16] or kv.dtype not in [torch.float16, torch.bfloat16]:
            raise ValueError(f"Expected float16/bfloat16, got q.dtype={q.dtype}, kv.dtype={kv.dtype}")
