...
class MSDeformAttnv2(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, n_subpoints=2, mixed_precision=False):
        ...
        # TODO : consider diverse options ------------------
        # option 1 : original
        # self.sampling_offsets = nn.Linear(2 * d_model, n_heads * 1 * n_points * n_subpoints * 2)
        # self.attention_weights = nn.Linear(2 * d_model, n_heads * 1 * n_points * n_subpoints)

        # option 4 : scale & sum
        self.sampling_offsets = nn.Linear(d_model, n_heads * n_levels * n_points * n_subpoints * 2)  # debug, 231120
        self.tanh = nn.Tanh()
    ...
    def _reset_parameters(self):
        ...
        grid_init = grid_init.view(1, 1, 1, self.n_subpoints, 2).\
            repeat(self.n_heads, self.n_levels, self.n_points, 1, 1) # n_heads n_levels n_points n_subpoints 2 debug, 231120
    ...
    def forward(self, query, input_flatten, input_global, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        sampling_offsets = self.tanh(sampling_offsets) * 0.25 # debug, 231120
        ...
        sampling_locations = reference_points + sampling_offsets # debug, 231120
        ...
        return output, (reference_points, sampling_offsets, attention_weights)

class MSDeformAttn3Dv2(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=8, mixed_precision=False):
        ...
        self.sampling_offsets = nn.Linear(2 * d_model, n_heads * 1 * n_points * 2)
        self.attention_weights = nn.Linear(2 * d_model, n_heads * 1 * n_points)
        self.value_proj = nn.Linear(d_model, d_model)
        self.output_proj = nn.Linear(d_model, d_model)
    ...
    def forward(self, query, reference_points, input_flatten, input_embeds, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        """
        :param query                       (N, Length_{query}, C)
        :param reference_points            (N, Length_{query}, n_levels, 2), range in [0, 1], top-left (0,0), bottom-right (1, 1), including padding area
                                        or (N, Length_{query}, n_levels, 4), add additional (w, h) to form reference boxes
        :param input_flatten               (N, \sum_{l=0}^{L-1} H_l \cdot W_l, C)
        :param input_embeds                n_levels x (N, Feat. Length, C), A List
        :param input_spatial_shapes        (n_levels, 2), [(H_0, W_0), (H_1, W_1), ..., (H_{L-1}, W_{L-1})]
        :param input_level_start_index     (n_levels, ), [0, H_0*W_0, H_0*W_0+H_1*W_1, H_0*W_0+H_1*W_1+H_2*W_2, ..., H_0*W_0+H_1*W_1+...+H_{L-1}*W_{L-1}]
        :param input_padding_mask          (N, \sum_{l=0}^{L-1} H_l \cdot W_l), True for padding elements, False for non-padding elements
        ...
        """
        For each image, corresponding queries generate offsets.
        When generating offsets, image information ('input_embeds') is used.        
        """
        # TODO : how to reflect reference points information here?
        sampling_offsets, attention_weights = [], []
        for _, lvl_feat in enumerate(input_embeds):
            # N (h w) dim -> N 1 dim -> N Len_q dim
            lvl_feat_repeat = torch.mean(lvl_feat, dim=1, keepdim=True).repeat(1, Len_q, 1) # N Len_q dim
            cat = torch.cat((query, lvl_feat_repeat), dim=-1)
            sampling_offsets.append(self.sampling_offsets(cat).view(N, Len_q, self.n_heads, 1, self.n_points, 2))
            attention_weights.append(self.attention_weights(cat).view(N, Len_q, self.n_heads, 1, self.n_points))
        sampling_offsets = torch.cat(sampling_offsets, dim=3)
        attention_weights = torch.cat(attention_weights, dim=3).view(N, Len_q, self.n_heads, -1)
        ...
        # Note, 230605: 'output_proj' should be applied to slots
        # output = self.output_proj(output)
        return output
...
class MSDeformAttn3D(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=8, mixed_precision=False):
        """
        Multi-Scale Deformable Attention Module. ** This is exactly the same as the official implementation. **
        :param d_model      hidden dimension
        :param n_levels     number of feature levels
        :param n_heads      number of attention heads
        :param n_points     number of sampling points per attention head per feature level
    ...
    def _reset_parameters(self):
        ...
        # TODO : where does 'with torch.no_grad()' come from?
        with torch.no_grad():
            self.sampling_offsets.bias = nn.Parameter(grid_init.view(-1))
