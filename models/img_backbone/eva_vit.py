...
from models.backbone.attention import FlashAttention, FlashMHA
...
class EVAViT(nn.Module):
    ...
    def load_pretrained(self, checkpoint_path):
        ...
        # Manually load pre-trained parameters from checkpoint
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        
        # Extract state_dict from checkpoint (handle different checkpoint formats)
        ...
        # Remove prefix if present (e.g., 'img_backbone.', 'backbone.', 'model.')
        model_dict = self.state_dict()
        
        # Try to match keys with and without prefix
        new_state_dict = {}
        for k, v in state_dict.items():
            # Remove common prefixes
            key = k
            for prefix in ['img_backbone.', 'backbone.', 'model.', 'module.']:
                if key.startswith(prefix):
                    key = key[len(prefix):]
                    break
            
            # Skip rope buffers (freqs_cos, freqs_sin) as they are computed based on image size
            # and may have different sizes between checkpoint and current model
            if 'rope' in key.lower() and ('freqs_cos' in key or 'freqs_sin' in key):
                continue
            
            # If key matches model's state_dict, use it
            if key in model_dict:
                # Check if shapes match
                if model_dict[key].shape == v.shape:
                    new_state_dict[key] = v
                else:
                    print(f"Skipping {key}: shape mismatch (checkpoint: {v.shape}, model: {model_dict[key].shape})")
